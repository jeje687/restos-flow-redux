...
export function setPlacesAction(places : {[key: string]: Place}) : SetPlacesAction {
...
export function setPlaces(location: Location, radius: number, language: string, pageToken: ?string){
    return async (dispatch: Dispatch, getState: GetState) => {
        try {
            let resp : ApiSearchResponse = await PlaceService.getPlaces(location, radius, language, pageToken);
            if(resp.results){
                const normalizedPlaces : {[key:string]:Place} = normalize(resp.results, placeSchema);
                dispatch(batchActions([
                    setPlacesAction(resp.results),
